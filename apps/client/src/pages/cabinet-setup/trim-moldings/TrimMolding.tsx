          ...
          <Form.Item label="Unit of Measure" name="unit_of_measure">
            <Select style={input80PercentStyle}>
              {["Each", "Per Linear Foot"].map((unit) => (
                <Select.Option value={unit} key={nanoid()}>
                  {unit}
                </Select.Option>
              ))}
            </Select>
    ...
    >
      ...
      <Form.Item label="# of Finished Slides" name="number_of_finished_sides">
        <Select style={{ width: "100px" }}>
          {[0, 1, 2].map((option) => (
            <Select.Option key={nanoid()} value={option}>
              {option}
            </Select.Option>
          ))}
        </Select>
    ...
    <Col span={6}>
      ...
      >
        ...
        <Form.Item label="Status" name="status">
          <Select allowClear>
            {ACTIVE_INACTIVE_STATUSES_OPTIONS.map((status) => (
              <Select.Option key={nanoid()} value={status.value}>
                {status.label}
              </Select.Option>
            ))}
          </Select>
        ...
        <Form.Item label="Classification" name="classification">
          <Select allowClear>
            {classifications?.map((classification) => (
              <Select.Option value={classification.value} key={nanoid()}>
                {classification.label}
              </Select.Option>
            ))}
          </Select>
        ...
        <Form.Item>
          <Table
            ...
            onRow={(row) => {
              return {
                onClick: () =>
                  navigate({ to: `/cabinet-setup/trim-moldings/${row.id}` }),
              };
            }}
          />
        </Form.Item>
