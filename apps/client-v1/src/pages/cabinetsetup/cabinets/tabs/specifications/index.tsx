...
import { inputNumberProps } from "../../../../../utilities";
...
export default function CabinetSpecifications() {
  ...
  const category = lowerCase(get(cabinet, "category", ""));
  const ref = useRef();
  ...
  useEffect(() => form.resetFields(), [cabinetCtx]);

  const CabinetComponent = useMemo(
    () =>
      getCabinetComponentByCategory(cabinetCtx.cabinet.category?.toLowerCase()),
    [cabinetCtx]
  );
  ...
  return (
    ...
    >
      ...
      <CabinetComponent
        // @ts-expect-error TS(2322): Type '{ category: string; cabinet: {}; ref: Mutabl... Remove this comment to see the full error message
        category={category}
        cabinet={cabinet}
        ref={ref}
        form={form}
