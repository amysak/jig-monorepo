                ...
                <Col span={12}>
                    <Form.Item label="Subclassification">
                        <CreatableSelect
                            ...
                            options={subclassifications?.map(
                                ({ subclassification }) => ({
                ...
                <Col span={6}>
                    <Form.Item label="Square Feet" name="square_feet">
                        <InputNumber
                            ...
                            addonAfter={
                                <RighTextAddon label="per linear foot" />
                            }
                        />
                    </Form.Item>
                </Col>
            ...
            <Form.Item shouldUpdate label="Discounted Material Cost">
                {() => {
                    const materialCost = safeNum(
                        form.getFieldValue('material_cost')
                    )
            ...
            <Form.Item shouldUpdate label="Material Cost with Waste Factor">
                {() => {
                    ...
                    const wasteFactor = safeNum(
                        form.getFieldValue('waster_factor')
                    )
                    const discountedMatCost = calcDiscountSalePrice(
                        materialCost,
                        suppliersDiscount
                    ...
                    return `$${toFixed(
                        discountedMatCost +
                            (discountedMatCost * wasteFactor) / 100
                    )}`
                }}
            </Form.Item>
                    ...
                    <Table
                        ...
                        onRow={(row) => {
                            return {
                                onClick: () =>
                                    navigate(
                                        `/cabinet-setup/trim-moldings/${row.id}`
                                    ),
                            }
                        }}
                    />
...
export default function TrimMolding() {
    ...
    const getSubclassifications = async (queryFilters = {}) => {
        try {
            ...
            const subclassifications = await getTrimMoldingSublassifications(
                query
            )
